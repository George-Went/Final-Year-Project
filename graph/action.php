...
if (isset($_POST["data_date"])) { // if the date is set 
  $data_date = $_POST['data_date'];
  $data_date = mysqli_real_escape_string($connect, $data_date);
}
if (isset($_POST["client"])) { // if a client has been selected
  $client = $_POST['client'];
...
$response = null; // set response to null (if action.php is used multiple times)
